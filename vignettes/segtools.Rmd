...
knitr::kable(seg_pairs_table(data = test_data, is_path = FALSE))
```
...
```{r seg_risk_grade_table-show, eval=FALSE}
...
```{r seg_risk_level_table-show, eval=FALSE}
seg_risk_level_table(data = test_data, is_path = FALSE)
```

```{r seg_risk_level_table-run, echo=FALSE}
knitr::kable(
seg_risk_level_table(data = test_data, is_path = FALSE)
)
```
...
6. `seg_binom_table()` returns a table binomial test for compliant pairs

```{r seg_binom_table-show, eval=FALSE}
seg_binom_table(data = test_data, is_path = FALSE)
```

```{r seg_binom_table-run, echo=FALSE}
knitr::kable(
seg_binom_table(data = test_data, is_path = FALSE)
)
```
